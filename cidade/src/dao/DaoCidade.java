...
public class DaoCidade {
     public static boolean inserir(Cidade objeto) {
        String sql = "INSERT INTO cidade (nome, sigla_estado, nr_habitantes, data_emancipacao, area_total,  distancia_da_capital) VALUES (?, ?, ?, ?, ?, ?)";
        try {
            ...
            ps.setDouble(6, objeto.getDistancia_da_capital());
            ps.executeUpdate();
            return true;
     ...
     public static List<Cidade> consultar() {
        ...
        String sql = "SELECT codigo, nome, sigla_estado, nr_habitantes, data_emancipacao, area_total, distancia_da_capital FROM cidade";
        ...
        try {
            ...
            while (rs.next()) {
                ...
                objeto.setDistancia_da_capital(rs.getDouble("distancia_da_capital"));
      ...
      public static boolean alterar(Cidade objeto) {
        String sql = "UPDATE cidade SET nome = ?, sigla_estado = ?, nr_habitantes = ?, data_emancipacao = ?, area_total = ?, distancia_da_capital = ? WHERE codigo=?";
        try {
            PreparedStatement ps = conexao.Conexao.getConexao().prepareStatement(sql);
            ps.setString(1, objeto.getNome()); 
            ps.setString(2, objeto.getSigla_estado());
      ...
      public static Cidade consultar(int primaryKey) {
        //editar o SQL conforme a entidade
        String sql = "SELECT codigo, nome, sigla_estado, nr_habitantes, data_emancipacao, area_total, distancia_da_capital FROM cidade WHERE codigo=?";
        PreparedStatement ps;
        try {
            ps = conexao.Conexao.getConexao().prepareStatement(sql);
